...
public final class EventInventoryCommand implements ICommand {
    ...
    public Optional<TypeMessage> execute() {
        ...
        var item1 = Cache.getItem(event.getIdItemNew());
        if (item1.isEmpty()) return Optional.of(TypeMessage.ITEM_ERRO_FOUND);
        this.newItem = item1.get();
    ...
    private Optional<TypeMessage> getEventTypeMessage() {
        return this.newItem.getId() == 1 ? Optional.of(TypeMessage.EVENT_INVENTORY_KEY) : Optional.empty();
    }
